...
(function(sip){
    ...
    //compare 2 Messages by to, from, Contact, Call-ID,Content-Length, Content-Type and Body
    Message.prototype.compare = function(message) {
		var result= this.from==message.from;
		result = result && this.to==message.to;

		result = result && this.body==message.body;
		result = result && this.getItem("Contact").toString()==message.getItem("Contact").toString();
		result = result && this.getItem("Call-ID").toString()==message.getItem("Call-ID").toString();
		result = result && this.getItem("Content-Length").toString()==message.getItem("Content-Length").toString();
		result = result && this.getItem("Content-Type").toString()==message.getItem("Content-Type").toString();
...
//            throw new String('More than one Via header in response');
    ...
    UserAgent.prototype.receivedRequest = function(transaction, request) {
        ...
        if (request.getItem("To")["tag"] === undefined) {
	            if (this.stack.findOtherTransaction(request, transaction)&&request.method!="BYE"&&request.method!="CANCEL") {
